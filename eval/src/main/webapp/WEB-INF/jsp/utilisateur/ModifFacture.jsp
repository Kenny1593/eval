        ...
        <div class="row">
            <div class="card">
                <div class="card-body">
                    ...
                    <form method="post" action="<%=request.getContextPath()%>/modFact">
                        <input type="hidden" name="id" value="<%=facture.getId()%>">
                        <div class="row mb-3">
                            <label class="col-sm-2 col-form-label">Date</label>
                            <div class="col-sm-10">
                                <input type="date" class="form-control" name="date" value="<%=facture.getDate()%>">
                            </div>
                        </div>
                        <div class="row mb-3">
                            <label class="col-sm-2 col-form-label">Ajout Type d'acte</label>
                            <div class="col-sm-10">
                                <select class="form-select" aria-label="Default select example" name="patient.id">
                                    <option value="<%= facture.getPatient().getId()%>"><%=facture.getPatient().getNom()%></option>
                                    <% for(Patient patient: patients){ %>
                                    <option value="<%=patient.getId()%>"><%=patient.getNom()%></option>
                                    <% } %>
                                </select>
                            </div>
                        </div>
